...
const User = {
  ...
  auth: (email, password, cb) => {
    const query = 'SELECT user_id, first_name, last_name, password FROM user WHERE email = ? LIMIT 1';
  ...
  findById: (id, cb) => {
    const query = 'SELECT first_name, last_name, email, org_id FROM user WHERE user_id = ?';
